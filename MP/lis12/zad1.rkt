...
(define-type Binding
  (bind [name : Symbol]
        [val : (-> Value)])) ;funkcja zamiast zawartosci trzymamy funkcje zeby obliczac na bierzaco
...
(define mt-env empty)
(define (extend-env [env : Env] [x : Symbol] [v : (-> Value)]) : Env ;tu to samo
  (cons (bind x v) env))
(define (lookup-env [n : Symbol] [env : Env]) : Value
  (type-case (Listof Binding) env
    ...
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                         ((bind-val b))] ; wywoluje funkcje a nie ja deklaruje
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
       (eval e2 (extend-env env x (lambda () (eval e1 env))))] ; obliczamy e2 w trakcie wykonywania a nie letujemy argument i go obliczamy przed wykonaniem
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) (lambda () (eval e2 env)))])) ; obliczamy e2 w trakcie wykonania a nie obliczamy i aplikujemy do przewartosciowanego argumentu

(define (apply [v1 : Value] [v2 : (-> Value)]) : Value ; zmieniamy na funkcje bo obliczamy w trakcjie wykonania a nie dajemy gotowa wartosc
  (type-case Value v1
    [(funV x b env)
