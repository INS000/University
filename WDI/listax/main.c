...
void zad1_m_to_l(int n){ //n - liczba wierzcholkow
    for(int i = 1; i <= n; i++){ //pamieciowa n^2    czasowa n^2
        for(int j = 1; j <= n; j++){
            if(tab[i][j] == 1)
...
void zad1_l_to_m(int n){ //n - liczba wierzcholkow
    for(int i = 1; i <= n; i++){ //pamieciowa n^2    czasowa m+n
...
bool zad2(int u, int v){// n - ilosc wierzcholkow
    ...
    del_vertex(v, u); // nie trzeba ale fajnie by usunac cala krawedz
    dfs(u);
    if(vis[v] == 1)
        return 0;
    return 1;
...
bool zad6(int v){
    ...
    while(pom != NULL){
        ...
        del_vertex(v, pom->val); //nie musimy usuwac krawedzi z obu stron
        pom = pom->next;
    }
    zeruj(103); //jedyne potrzebne
